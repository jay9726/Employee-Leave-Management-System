...
////            return new AuthResponseDTO { Token = token, Email = responseDTO.Email, FullName = responseDTO.FullName };
...
////            return new AuthResponseDTO { Id = user.Id, departmentId = user.DepartmentId , Token = token, Email = responseDTO.Email, FullName = user.FullName, Role = role, ExpiresAt = ExpiresAt, ImagePath = user.ImagePath};
...
//        public AuthService(IUnitOfWork unitOfWork, UserManager<Employee> userManager)   
//        {
...
//                bool created = await _unitOfWork.UserRepository.CreateUserAsync(user, responseDTO.Password!);
...
//            try
//            {
//                var user = await _unitOfWork.UserRepository.GetUserByEmailAsync(responseDTO.Email!);

//                if (user == null) return null;

//                var checkPassword = await _unitOfWork.UserRepository.CheckPasswordAsync(user, responseDTO.Password!);
